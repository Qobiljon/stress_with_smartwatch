...
mobility_filter_dir = '{0}/{1}'.format(root_dir, '9. mobility-filter-v2')
# id->value maps
user_id_email_map = {
...
for email in emails:
    ...
    _feature_window_size = feature_window_sizes[email]
    ...
    with open('{0}/{1}.csv'.format(mobility_filter_dir, email), 'w+') as w:
        ...
        for ground_truth in ground_truths:
            ...
            till_timestamp = timestamp - (1800000 if is_self_report else _feature_window_size)
            while timestamp > till_timestamp:
                ...
                if selected_rr_intervals is not None and selected_acc_values is not None:
                    try:
                        ...
                        if not is_acc_window_active(acc_values=selected_acc_values, from_ts=timestamp, threshold=acc_threshold):
