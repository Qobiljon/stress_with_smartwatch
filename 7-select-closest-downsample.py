...
no_filter_dir = '{0}/{1}'.format(root_dir, '7. select-closest-downsampled')
# id->value maps
...
def select_downsample_data(dataset, from_ts, till_ts):
  ...
  for row in dataset:
    if from_ts <= row[0] < till_ts:
      ...
      break
  timestamps = [ts for ts in range(from_ts, till_ts, 500)]
  res = []
    ...
    with open('{0}/{1}.csv'.format(no_filter_dir, email), 'w+') as w:
        ...
        for ground_truth in ground_truths:
            ...
            while timestamp > till_timestamp:
                selected_rr_intervals = select_downsample_data(
