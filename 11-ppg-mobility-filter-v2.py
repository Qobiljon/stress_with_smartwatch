...
import os, csv, datetime, math, statistics
import numpy as np
...
mobility_filter_dir = '{0}/{1}'.format(root_dir, '11. ppg+mobility-filter-v2')
# id->value maps
user_id_email_map = {
    '19': 'salman@nsl.inha.ac.kr',
    '20': 'jumabek4044@gmail.com',
    '18': 'jskim@nsl.inha.ac.kr',
    '11': 'aliceblackwood123@gmail.com',
    '10': 'laurentkalpers3@gmail.com',
    '7': 'nazarov7mu@gmail.com',
    '1': 'nslabinha@gmail.com',
    '8': 'azizsambo58@gmail.com',
    '3': 'nnarziev@gmail.com',
    '6': 'mr.khikmatillo@gmail.com'
}
...
sensing_window_sizes = {
    ...
    'nslabinha@gmail.com' : 1800000,
    'azizsambo58@gmail.com' : 1800000,
...
ppg_thresholds = {
    ...
    'laurentkalpers3@gmail.com' : 90000,
...
def load_ppg_data(parent_dir, participant_email):
    res = []
    file_path = os.path.join(parent_dir, participant_email, 'LIGHT_INTENSITY.csv')
    if not os.path.exists(file_path):
        return None
    with open(file_path, 'r') as r:
        for line in r:
...
def is_acc_window_active(acc_values, from_ts, threshold):
    activeness_scores = []
    magnitudes = []
    till_ts = from_ts + acc_window_size
    term_ts = from_ts + 60000
    ...
    for activeness_score in activeness_scores:
        #if activeness_score > (low_limit + threshold * activeness_range):
        if activeness_score > threshold:
            active_count += 1
...
def get_acc_window_activeness_score(acc_values, from_ts):  
    activeness_scores = []
    magnitudes = []
    till_ts = from_ts + acc_window_size
    term_ts = from_ts + 60000
    ...
    for activeness_score in activeness_scores:
        ...
        if activeness_score > activity_threshold:
            active_part_scores += [activeness_score]
    if len(active_part_scores) > len(activeness_scores) / 2:
        return 1, sum(active_part_scores) / len(active_part_scores)
    else:
        return 0, sum(activeness_scores) / len(activeness_scores)
...
for email in emails:
    ...
    sensing_window_size = sensing_window_sizes[email]
    ppg_threshold = ppg_thresholds[email]
    acc_threshold = acc_thresholds[email]
    with open('{0}/{1}.csv'.format(mobility_filter_dir, email), 'w+') as w:
        w.write('timestamp,ppg_stdev,activeness_scores\n')
        participants_rr_dataset = load_rr_data(parent_dir=sep_dir, participant_email=email)
        participants_acc_dataset = load_acc_data(parent_dir=sep_dir, participant_email=email)
        participants_ppg_dataset = load_ppg_data(parent_dir=sep_dir, participant_email=email)
        ...
        for ground_truth in ground_truths:
            ...
            till_timestamp = timestamp - (1800000 if is_self_report else sensing_window_size)
            while timestamp > till_timestamp:
                ...
                selected_acc_values = select_data(
                    dataset=participants_acc_dataset,
                    from_ts=timestamp - feature_window_size,
                    till_ts=timestamp,
                    with_timestamp=True
                )
                selected_ppg_values = select_data(
                    dataset=participants_ppg_dataset,
                    from_ts=timestamp - feature_window_size,
                    till_ts=timestamp
                )
                timestamp -= feature_window_size
                if selected_rr_intervals is not None and selected_acc_values is not None and selected_ppg_values is not None:
                    try:
                        print('processing :', timestamp, email)
                        ppg_stdev = get_ppg_stdev(selected_ppg_values)
                        mobility_label, mobility_score = get_acc_window_activeness_score(selected_acc_values, timestamp)
                        if get_ppg_stdev(ppg_values=selected_ppg_values) < ppg_threshold and not is_acc_window_active(acc_values=selected_acc_values, from_ts=timestamp, threshold=acc_threshold):
...
for email in emails:
    ...
    with open('{0}/{1}.csv'.format(mobility_filter_dir, email), 'r') as r:
        lines = r.readlines()[1:]
